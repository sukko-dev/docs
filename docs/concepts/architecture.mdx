...
Sukko is a multi-service platform that ingests messages from a pluggable message backend (Kafka/Redpanda, NATS JetStream, or direct) and delivers them to clients via WebSocket, SSE, and push notifications (FCM, APNs, Web Push).
...
| **Push** | Push notification service. Consumes from Kafka or NATS, dispatches to FCM, APNs, and Web Push. Requires a persistent backend (not direct). | 3009 |
...
External API / Producer
        │
        ▼
  Message Backend             Kafka/Redpanda, NATS JetStream, or direct
        │
        ▼
  Core: Consumer Pool         consume, rate limit, CPU backpressure
        │
        ▼
  Core: Broadcast Bus         NATS or Valkey, fan-out across all replicas
        │
   ┌────┼────┐
   ▼    ▼    ▼
...
```
  Message Backend             (Kafka/Redpanda or NATS JetStream)
        │
        ▼
...
The broadcast bus (Valkey by default, or NATS, configurable via `BROADCAST_TYPE`) handles inter-instance message distribution:
